...
**当前版本：v1.9.45（YYDS 域名留空自动随机 · 注册配置多 worker 同步）**
...
```bash
docker pull ghcr.io/hm2899/grokcli-2api:1.9.45
# 或
...
services:
  ...
  grokcli-2api:
    image: ghcr.io/hm2899/grokcli-2api:1.9.45
    ports:
...
git tag v1.9.45
git push origin v1.9.45
```

成功后可拉取（**必须小写**）：

- `ghcr.io/hm2899/grokcli-2api:1.9.45`
...
> 镜像 tag 与 `app.py` 中 `APP_VERSION` 一致（当前 **1.9.45**）。推 `main` 会打 `edge` 与版本号；打 `v1.9.45` tag 会额外发布 `latest`。  
