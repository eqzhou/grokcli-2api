...
**当前版本：v1.9.25（高并发 hybrid · 内联本地过盾）**
...
```bash
docker pull ghcr.io/hm2899/grokcli-2api:1.9.25
# 或
...
services:
  ...
  grokcli-2api:
    image: ghcr.io/hm2899/grokcli-2api:1.9.25
    ports:
      - "3000:3000"
    environment:
...
- `ghcr.io/hm2899/grokcli-2api:1.9.25`
...
- **v1.9.25**（当前）：注册会话跨 worker 进度可见；状态卡片 soft-nav 不丢失；支持停止注册
- **v1.9.24**：修复注册/导入后账号池总数不刷新；账号列表服务端排序（默认最新在上）；开发态 `GROK2API_RELOAD` / `./dev.sh` 热更新
- **v1.9.23**：协议注册支持 **主容器内联本地过盾**（`turnstile-solver` 子模块）；管理台过盾方式可选本地/YesCaptcha，本地模式不展示 URL；compose/启动支持 `TURNSTILE_THREAD` 打码线程参数
...
> 镜像 tag 与 `app.py` 中 `APP_VERSION` 一致（当前 **1.9.25**）。推 `main` 会打 `edge` 与版本号；打 `v1.9.25` tag 会额外发布 `latest`。  
