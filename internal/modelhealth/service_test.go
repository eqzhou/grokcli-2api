...
func TestStopWaitsForLoopAndAllowsCleanRestart(t *testing.T) {
	...
	service.Start()
	service.RequestRunSoon()
	waitForModelHealthRun(t, runEntered, 1)
	...
	waitForModelHealthStop(t, stopDone)
	if returnedEarly {
	...
	service.Start()
	service.RequestRunSoon()
	waitForModelHealthRun(t, runEntered, 2)
	service.Stop()
...
func waitForModelHealthRun(t *testing.T, runs <-chan int32, want int32) {
	t.Helper()
	select {
	case got := <-runs:
		if got != want {
...
func waitForModelHealthStop(t *testing.T, done <-chan struct{}) {
	t.Helper()
	select {
	case <-done:
	case <-time.After(time.Second):
