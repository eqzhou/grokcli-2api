...
(function (G2A) {
  ...
  function renderConnList(hostId, st, health) {
    const host = $(hostId);
    if (!host) return;
    const healthOk = !!(health && (health.ready || health.ok || health.status === "ok"));
    const rows = [
      {
        name: "管理权限",
        ok: st ? !st.setup_needed : false,
        warn: !!(st && st.setup_needed),
        desc: st ? (st.setup_needed ? "需要初始化" : "密码已配置") : "正在确认",
      },
      {
        name: "服务连接",
        ok: healthOk,
        warn: !health,
        desc: healthOk ? "API 与数据服务正常" : (health ? "连接检查未通过" : "正在确认"),
      },
    ];
  ...
  async function initLoginPage() {
    ...
    try {
      ...
      if ($("boot-hint")) $("boot-hint").textContent = `请确认服务已启动：${e.message}`;
      G2A.toast("无法连接服务: " + e.message, false);
      return;
    }
    try { renderConnList("auth-conn", st, health); } catch (_) {}

    // The probe is transient; the login panel remains the only page surface.
