          ...
          try {
            ...
            if (q.pool && typeof q.pool === "object") {
              ...
              qPatch = {
                ...
                disabled_reason: dead ? (q.exhaust_reason || q.error || "额度耗尽") : null,
              };
            }
...
async function refreshAllQuota(force = true) {
  ...
  try {
    ...
    rows.forEach((q) => {
      if (q && typeof q === "object" && q.account_id) quotaCache[q.account_id] = q;
    });
    // Hot-update current page rows with full pool status (quota_disabled / re-enable).
    (accountsList || []).forEach((a) => {
      const q = a && a.id != null ? quotaCache[a.id] : null;
      if (!q || typeof q !== "object") return;
    ...
    if (qs) {
      ...
      const exhausted = data.exhausted_count ?? rows.filter(x => x && (x.exhausted || x.auto_disabled || x.disabled_for_quota)).length;
...
function startAutoUiRefresh() {
  ...
  uiRefreshTimer = setInterval(async () => {
    try {
      ...
      // Gateway HTML 502/503/504 or network: back off so we don't hammer a dead/slow proxy.
      ...
      if (gateway || network) {
        ...
        const waitMs = Math.min(120000, 8000 * Math.pow(2, Math.max(0, _autoRefreshFailStreak - 1)));
  ...
  try {
    ...
    if (btn.dataset.act === "quota-one") {
      ...
      try {
        ...
        // Always keep the live quota payload on the row (pool may omit last_quota to avoid JSON cycles).
        if (!qPatch.last_quota) qPatch.last_quota = q;
        applyAccountLivePatch(id, { _pool: qPatch });
        try { renderAccountStatusChips(); } catch (_) {}
        try { renderStats(); } catch (_) {}
  ...
  try {
    ...
    $("usage-events-tbody").innerHTML = items.map((it) => {
      ...
      return `<tr data-usage-detail='${detailAttr}' style="cursor:pointer" title="点击查看完整明细">
        ...
        <td class="mono" title="计费 = total − cache_read">${fmtNum(billedTok)}${cacheRead > 0 ? `<div class="g2a-muted" style="font-size:11px">原 ${fmtNum(totalTok)}</div>` : ""}</td>
