...
**当前版本：v1.9.44（任务日志 · JSON/SSO 进度 · hybrid 内联过盾）**
...
```bash
docker pull ghcr.io/hm2899/grokcli-2api:1.9.44
# 或
...
services:
  ...
  grokcli-2api:
    image: ghcr.io/hm2899/grokcli-2api:1.9.44
    ports:
...
git tag v1.9.44
git push origin v1.9.44
```

成功后可拉取（**必须小写**）：

- `ghcr.io/hm2899/grokcli-2api:1.9.44`
...
> 镜像 tag 与 `app.py` 中 `APP_VERSION` 一致（当前 **1.9.44**）。推 `main` 会打 `edge` 与版本号；打 `v1.9.44` tag 会额外发布 `latest`。  
