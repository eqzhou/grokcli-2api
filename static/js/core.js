...
function setRegStatusText(text) {
  ...
  if (next !== regLastStatusText || (el.textContent || "") !== next) {
    regLastStatusText = next;
    el.textContent = next;
  }
  // Color status tag by semantic state (supports long batch summary strings).
  const low = next.toLowerCase();
  let state = "idle";
  if (/stopping|停止中/.test(low)) state = "stopping";
  else if (/cancelled|stopped|已停|停止\s/.test(low)) state = "cancelled";
  else if (/imported|success|completed|done|成功\s*\d/.test(low) && !/失败\s*[1-9]/.test(low) && !/运行|进行/.test(low)) state = "imported";
  else if (/error|failed|partial|失败\s*[1-9]|不可用/.test(low)) state = "error";
  else if (/starting|running|register|solving|waiting|probing|restor|queued|进行|运行|测活|启动/.test(low)) state = "running";
  else if (/not found|已结束|idle|—/.test(low)) state = low.includes("结束") ? "imported" : "idle";
  el.setAttribute("data-reg-state", state);
  el.classList.add("g2a-reg-status-tag");
  paintRegChrome({ statusText: next, state });
}

function paintRegChrome(opts = {}) {
  const state = opts.state || "idle";
  const live = $("reg-live-dot");
  if (live) {
    live.classList.remove("is-live", "is-ok", "is-bad", "is-stop", "is-warn");
    if (state === "running") live.classList.add("is-live");
    else if (state === "imported") live.classList.add("is-ok");
    else if (state === "error") live.classList.add("is-bad");
    else if (state === "cancelled") live.classList.add("is-stop");
    else if (state === "stopping") live.classList.add("is-warn");
  }
  // Header stop button enabled while a job is tracked or actively running.
  const hasTrack = !!(regBatchId || regSessionId || (regSessionIds && regSessionIds.length));
  const stopable = hasTrack || state === "running" || state === "stopping";
  const stopBtn = $("btn-stop-reg");
  if (stopBtn && !regStopping) stopBtn.disabled = !stopable;
  const stopInline = $("btn-stop-reg-inline");
  if (stopInline && !regStopping) stopInline.disabled = !stopable && state === "idle";
}

function normalizeRegProgress(stats = {}) {
  const totalRaw = Number(stats.total != null ? stats.total : (stats.count != null ? stats.count : stats.spawned)) || 0;
  const success = Math.max(0, Number(stats.success != null ? stats.success : (stats.imported != null ? stats.imported : stats.ok)) || 0);
  const running = Math.max(0, Number(stats.running != null ? stats.running : stats.inflight) || 0);
  const probing = Math.max(0, Number(stats.probing != null ? stats.probing : stats.probe_pending_count) || 0);
  const cancelled = Math.max(0, Number(stats.cancelled != null ? stats.cancelled : (stats.cancelled_count != null ? stats.cancelled_count : stats.stop)) || 0);
  const hasUnattempted = stats.unattempted != null || stats.not_attempted != null || stats.remaining != null;
  let unattempted = Math.max(0, Number(stats.unattempted != null ? stats.unattempted : (stats.not_attempted != null ? stats.not_attempted : stats.remaining)) || 0);
  const status = String(stats.status || stats.batch_status || "").toLowerCase();
  const terminal = ["done", "partial", "error", "failed", "cancelled", "stopped", "completed"].includes(status);
  let fail = Math.max(0, Number(stats.fail != null ? stats.fail : (stats.error != null ? stats.error : stats.failed)) || 0);
  if (!hasUnattempted && terminal && totalRaw > 0) {
    unattempted = Math.max(0, totalRaw - success - fail - running - cancelled);
  }
  if ((hasUnattempted || terminal) && totalRaw > 0) {
    fail = Math.min(fail, Math.max(0, totalRaw - success - running - cancelled - unattempted));
  }
  const total = Math.max(totalRaw, success + fail + running + cancelled + unattempted);
  return { total, success, fail, running, probing, cancelled, unattempted };
}

function setRegStats(stats = {}) {
  const progress = normalizeRegProgress(stats);
  const total = progress.total;
  const ok = progress.success;
  const fail = progress.fail;
  const run = progress.running;
  const probing = progress.probing;
  const stop = progress.cancelled;
  const unattempted = progress.unattempted;
  const map = {
    "reg-stat-total": total,
    "reg-stat-ok": ok,
    "reg-stat-fail": fail,
    "reg-stat-run": run,
    "reg-stat-probing": probing,
    "reg-stat-stop": stop,
    "reg-stat-unattempted": unattempted,
  };
  ...
  const status = String(stats.status || stats.batch_status || "").toLowerCase();
  const terminal = ["done", "partial", "error", "failed", "cancelled", "stopped", "completed"].includes(status);
  const done = ok + fail + stop + (terminal ? unattempted : 0);
  const pct = total > 0 ? Math.max(0, Math.min(100, Math.round((done / total) * 100))) : (run > 0 ? 8 : 0);
  const fill = $("reg-progress-fill");
  const bar = $("reg-progress-bar");
  const wrap = $("reg-progress-wrap");
  const pctEl = $("reg-progress-pct");
  const label = $("reg-progress-label");
  if (fill) fill.style.width = pct + "%";
  if (bar) bar.setAttribute("aria-valuenow", String(pct));
  if (pctEl) pctEl.textContent = pct + "%";
  if (label) {
    if (regStopping && run > 0) label.textContent = "停止中，等待进行中的任务退出…";
    else if (total <= 0 && run <= 0) label.textContent = "等待开始…";
    else if (run > 0 || probing > 0) label.textContent = `进行中 ${run}` + (probing ? ` · 测活中 ${probing}` : "") + ` · 已完成 ${done}/${total || done}`;
    else if (fail > 0 && ok > 0) label.textContent = `部分完成 · 成功 ${ok} · 失败 ${fail}`;
    else if (fail > 0 && ok <= 0) label.textContent = `已结束 · 全部失败 ${fail}`;
    else if (stop > 0 && ok + fail <= 0) label.textContent = `已停止 · 已取消 ${stop}` + (unattempted ? ` · 未尝试 ${unattempted}` : "");
    else if (unattempted > 0 && run <= 0) label.textContent = `已结束 · 未尝试 ${unattempted}`;
    else label.textContent = total ? `已完成 ${done}/${total}` : "已完成";
  }
  if (wrap) {
    wrap.classList.remove("is-done", "is-error", "is-stopping");
    if (regStopping && run > 0) wrap.classList.add("is-stopping");
    else if (total > 0 && done >= total && fail > 0 && ok <= 0) wrap.classList.add("is-error");
    else if (total > 0 && done >= total) wrap.classList.add("is-done");
    else if (fail > 0 && run <= 0 && ok > 0) wrap.classList.add("is-error");
  }
}

function updateRegModeChip(cfg) {
  const el = $("reg-mode-chip");
  if (!el) return;
  const c = cfg || {};
  const count = Math.max(1, Number(c.count || 1) || 1);
  const workers = Math.max(1, Number(c.concurrency || 1) || 1);
  const captcha = String(c.captcha_provider || c.captcha || "local").toLowerCase() === "yescaptcha"
    ? "YesCaptcha"
    : "本地过盾";
  const mail = String(c.mail_provider || "moemail");
  const mode = count > 1 ? `批量 ×${count}` : "单次";
  el.textContent = `${mode} · ${workers} 线程 · ${captcha} · ${mail}`;
}
  ...
  try {
    ...
    try {
      ...
      if (page === "models" && typeof loadModels === "function") {
        ...
        try { bindRegSettingsDialog(); } catch (_) {}
        try {
...
function hideEmptyLogPanels() {
  ...
  if (regBox) {
    // Registration controls live in this workbench, so it remains visible at idle.
...
function rebindPageControls() {
  ...
  if ($("btn-start-reg")) on("btn-start-reg", "onclick", async () => {
    try {
      ...
      showPanel("reg-session-box");
      setRegStats({ total: startedCount, success: 0, fail: 0, running: Math.min(workers, startedCount), cancelled: 0 });
  ...
  if ($("btn-save-reg")) on("btn-save-reg", "onclick", async () => {
    try {
      await saveRegConfig();
      closeRegSettingsDialog();
    } catch (_) {}
  });
  ...
  try { bindRegSettingsDialog(); } catch (e) { console.warn("bindRegSettingsDialog", e); }
...
async function loadDashboard() {
  ...
  try {
    ...
    const statusAccounts = st.accounts || (dashCache && dashCache.accounts) || {};
    const statusPool = st.pool || (dashCache && dashCache.pool) || {};
    const total = statusPool.total ?? statusAccounts.account_count ?? 0;
    const live = statusPool.live ?? statusPool.enabled ?? statusAccounts.active_count ?? 0;
    renderRuntimeStatusPill({
      total,
      live,
      credentialsOk: !!st.credentials_ok || Number(live || 0) > 0,
      email: st.credentials_email || "",
    });
  } catch (_) {}
}
...
function renderRuntimeStatusPill({ total = 0, live = 0, credentialsOk = false, email = "" } = {}) {
...
function renderStats() {
  ...
  const poolTotal = pool.total ?? acc.account_count ?? 0;
  const poolLive = pool.live ?? pool.enabled ?? acc.active_count ?? 0;
  const credOk = !(d.credentials && d.credentials.error) && (s.credentials_ok || poolLive > 0);
  renderRuntimeStatusPill({
    total: poolTotal,
    live: poolLive,
    credentialsOk: credOk,
    email: s.credentials_email || "",
  });
  ...
  const apiBase = d.api_base || s.api_base || "—";
  const cliVer = d.cli_version || s.cli_version || "—";
  const upstream = d.upstream || s.upstream || "—";
  const keysEn = keys.enabled ?? 0;
  const keysTot = keys.total ?? 0;
  const usage = d.usage || s.usage || {};
  const todayTok = usage.today_tokens || 0;
  const todayReq = usage.today_requests ?? 0;
  const totalTok = usage.total_tokens ?? 0;
  const tmRunning = !!(tm.running || tm.cluster_running || tm.leader_running);
  const tmLabel = tmRunning
    ? "运行中"
    : (tm.enabled === false ? "已关闭" : (tm.enabled ? "已启用" : "未运行"));
  const tmTone = tmRunning ? "ok" : (tm.enabled === false ? "warn" : (tm.enabled ? "ok" : "bad"));
  const poolTone = Number(poolLive) > 0 ? "ok" : "warn";
  const keysTone = Number(keysEn) > 0 ? "ok" : "warn";

  $("stats-grid").innerHTML = `
    <div class="stat g2a-stat-card" data-stat="api">
      <div class="g2a-stat-top"><span class="label">API Base</span><span class="g2a-stat-chip">endpoint</span></div>
      <div class="value mono g2a-stat-value-sm" title="${esc(apiBase)}">${esc(apiBase)}</div>
      <div class="sub">对外兼容 OpenAI / Anthropic</div>
    </div>
    <div class="stat g2a-stat-card" data-stat="cli">
      <div class="g2a-stat-top"><span class="label">CLI 版本</span><span class="g2a-stat-chip">build</span></div>
      <div class="value mono">${esc(cliVer)}</div>
      <div class="sub">上游 <span class="mono">${esc(upstream)}</span></div>
    </div>
    <div class="stat g2a-stat-card is-${poolTone}" data-stat="pool">
      <div class="g2a-stat-top"><span class="label">账号池</span><span class="g2a-stat-chip">pool</span></div>
      <div class="value g2a-stat-metric">
        <span class="g2a-stat-num">${esc(String(poolLive))}</span>
        <span class="g2a-stat-unit">可轮询</span>
        <span class="g2a-stat-sep">/</span>
        <span class="g2a-stat-num is-dim">${esc(String(poolTotal))}</span>
        <span class="g2a-stat-unit is-dim">总量</span>
      </div>
      <div class="sub">模式 ${esc(d.account_mode || s.account_mode || "—")} · 冷却 ${pool.in_cooldown ?? 0} · 过期 ${pool.expired ?? 0} · 封禁 ${pool.model_blocked ?? 0} · 额度 ${pool.quota_disabled ?? 0} · 禁用 ${pool.disabled ?? 0}</div>
    </div>
    <div class="stat g2a-stat-card is-${keysTone}" data-stat="keys">
      <div class="g2a-stat-top"><span class="label">API Keys</span><span class="g2a-stat-chip">auth</span></div>
      <div class="value g2a-stat-metric">
        <span class="g2a-stat-num">${esc(String(keysEn))}</span>
        <span class="g2a-stat-unit">启用</span>
        <span class="g2a-stat-sep">/</span>
        <span class="g2a-stat-num is-dim">${esc(String(keysTot))}</span>
      </div>
      <div class="sub">请求累计 ${fmtNum(keys.total_requests ?? 0)} · 鉴权 ${keys.auth_required ? "开启" : "关闭"}</div>
    </div>
    <div class="stat g2a-stat-card is-ok" data-stat="usage">
      <div class="g2a-stat-top"><span class="label">今日用量</span><span class="g2a-stat-chip">tokens</span></div>
      <div class="value mono g2a-stat-hero">${fmtTokens(todayTok)}</div>
      <div class="sub">请求 ${fmtNum(todayReq)} · 累计 ${fmtTokens(totalTok)}（已扣缓存）</div>
    </div>
    <div class="stat g2a-stat-card is-${tmTone}" data-stat="token">
      <div class="g2a-stat-top"><span class="label">Token 自动续期</span><span class="g2a-stat-chip">cron</span></div>
      <div class="value">${esc(tmLabel)}</div>
      <div class="sub">最短剩余 ${esc(remLabel)} · 下次 ${nextWait ?? "—"}s${lastRef != null ? ` · 上次刷新 ${lastRef}` : ""}${lastTm.at ? ` · ${fmtTime(lastTm.at)}` : ""}</div>
    </div>`;
}
...
function openRegSettingsDialog() {
  const dialog = $("reg-settings-dialog");
  if (!dialog) return;
...
function bindRegSettingsDialog() {
  on("btn-open-reg-settings", "onclick", () => openRegSettingsDialog());
  on("btn-close-reg-settings", "onclick", () => closeRegSettingsDialog());
  on("btn-cancel-reg-settings", "onclick", () => closeRegSettingsDialog());
  on("reg-settings-dialog", "oncancel", () => {
    document.body.classList.remove("g2a-dialog-open");
  });
  on("reg-settings-dialog", "onclose", () => {
    document.body.classList.remove("g2a-dialog-open");
  });
  on("reg-settings-dialog", "onclick", (event) => {
    if (event && event.target === $("reg-settings-dialog")) closeRegSettingsDialog();
  });
}
...
function resetRegProgressForNewTask() {
  ...
  setRegStatusText("starting");
  setRegEmailText("—");
  setRegStats({ total: 0, success: 0, fail: 0, running: 0, cancelled: 0 });
  paintRegChrome({ state: "running" });
}
...
if (!window.__g2aRegMailDelegated) {
  ...
  document.addEventListener(
    ...
    (e) => {
      ...
      if (
        t.id === "reg-count" ||
        t.id === "reg-concurrency" ||
        t.id === "reg-captcha-provider" ||
        t.id === "reg-mail-provider"
      ) {
        try { updateRegModeChip(readRegConfig()); } catch (_) {}
      }
    },
    true
  );
...
function applyRegConfig(cfg) {
  ...
  paintRegMailFieldsToInput();
  try { updateRegModeChip(cfg); } catch (_) {}
  if ($("reg-expiry-ms")) {
...
const REG_TERMINAL_OK = new Set(["success", "completed", "imported", "done"]);
...
function adoptRegSessions(sessions, { batch = null, continuePolling = true } = {}) {
  ...
  const batchTerminal = ["done", "partial", "error", "cancelled", "stopped", "failed", "completed"].includes(batchStatus);
  const batchRunning = Number((batchObj && batchObj.running) || 0) > 0;
  const batchProgress = normalizeRegProgress(batchObj || {});
  const batchProbeRunning = batchProgress.probing > 0;
  const batchResolved = batchProgress.success + batchProgress.fail + batchProgress.cancelled + (batchTerminal ? batchProgress.unattempted : 0);
  const batchDone =
    !!batchObj &&
    !batchProbeRunning &&
    (batchStatus === "done" ||
      ...
      (Number(batchObj.done || 0) > 0 &&
        ...
        !batchRunning) ||
      (batchResolved > 0 &&
        batchResolved >= batchProgress.total &&
        !batchRunning));
  const allTerminal =
    list.length > 0 &&
    list.every((s) => isRegTerminalStatus(regStatusOf(s)));
  const finished = !!wasFinished || !!batchDone || (allTerminal && !batchRunning && !batchProbeRunning);
  ...
  if (list.length <= 1 && !regBatchId) {
    ...
    showRegSessionGroup(list.length ? list : (finished ? [] : placeholderSessions), {
      batch: batchObj,
    });
  }
  ...
  if (finished) {
    ...
    try {
      const total = Math.max(batchProgress.total, regSessionIds.length || 0);
      const done = Number((batchObj && batchObj.done) || batchResolved || total || 0);
      const ok = batchProgress.success;
      const fail = batchProgress.fail;
      const cancelled = batchProgress.cancelled;
      const unattempted = batchProgress.unattempted;
      setRegStatusText(
        total > 0
          ? `已结束 · ${done}/${total}` +
            (ok || fail ? ` · 成功 ${ok} / 失败 ${fail}` : "") +
            (cancelled ? ` · 已取消 ${cancelled}` : "") +
            (unattempted ? ` · 未尝试 ${unattempted}` : "")
          : "已结束"
      );
      setRegEmailText(regBatchId ? `batch ${regBatchId}` : (regSessionId || "—"));
    } catch (_) {}
...
function summarizeRegSessions(sessions) {
  ...
  for (const s of list) {
    ...
    const probePending = !!(s && s.probe && s.probe.pending);
    if (probePending) {
      const importedIds = Array.isArray(s.imported_account_ids) ? s.imported_account_ids : [];
      probing += Math.max(1, importedIds.length);
    }
    if (REG_TERMINAL_OK.has(st)) success += 1;
    else if (st === "cancelled" || st === "stopped") cancelled += 1;
    else if (REG_TERMINAL_BAD.has(st)) fail += 1;
    else if (st === "probing" && !probePending) probing += 1;
    else running += 1;
  }
  return {
    total: list.length,
...
function buildRegLogText(sessions, { batch = null, extraLines = [] } = {}) {
  ...
  const progress = normalizeRegProgress(Object.assign({}, batch || {}, {
    total: Math.max(
      stats.total,
      Number((batch && (batch.total || batch.count || batch.spawned)) || 0) || 0,
      Array.isArray(batch && batch.session_ids) ? batch.session_ids.length : 0
    ),
    success: Math.max(stats.success, Number((batch && (batch.imported ?? batch.success)) || 0) || 0),
    fail: Math.max(stats.fail, Number((batch && (batch.error ?? batch.failed)) || 0) || 0),
    running: Math.max(stats.running, Number((batch && batch.running) || 0) || 0),
    probing: Math.max(stats.probing, Number((batch && (batch.probing ?? batch.probe_pending_count)) || 0) || 0),
    cancelled: Math.max(stats.cancelled, Number((batch && (batch.cancelled ?? batch.cancelled_count)) || 0) || 0),
  }));
  const { total, success, fail, running, probing, cancelled, unattempted } = progress;
  const lines = [];
  lines.push("======== 协议注册进度 ========");
  ...
  if (batch && batch.stop_reason) {
    lines.push(`自动停止: ${String(batch.stop_reason)}`);
  }
  if (batch && batch.rate_limit_reset_at) {
    const resetMs = Number(batch.rate_limit_reset_at) * 1000;
    const resetText = Number.isFinite(resetMs) ? new Date(resetMs).toLocaleString() : String(batch.rate_limit_reset_at);
    lines.push(`限流恢复时间: ${resetText}`);
  }
...
function showRegSession(s, opts = {}) {
  ...
  const cancelledN = (s && (regStatusOf(s) === "cancelled" || regStatusOf(s) === "stopped")) ? 1 : 0;
  setRegStats({
    ...
    cancelled: Math.max(stats.cancelled, cancelledN),
    unattempted: 0,
  });
...
function showRegSessionGroup(sessions, opts = {}) {
  ...
  setRegStats({
    ...
    cancelled,
    unattempted,
    status: (batch && (batch.batch_status || batch.status)) || "",
  });
...
async function pollRegSession() {
  ...
  try {
    ...
    const batchProgress = normalizeRegProgress(batch || {});
    const batchProbeRunning = batchProgress.probing > 0;
    const batchResolved = batchProgress.success + batchProgress.fail + batchProgress.cancelled + (batchTerminal ? batchProgress.unattempted : 0);
    const batchDone =
      batch &&
    ...
    saveRegTrack();
    const finalProgress = normalizeRegProgress(Object.assign({}, batch || {}, {
      total: targetTotal,
      success: Math.max(stats.success, batchProgress.success),
      fail: Math.max(stats.fail, batchProgress.fail),
      running: 0,
      probing: 0,
      cancelled: Math.max(stats.cancelled, batchProgress.cancelled),
    }));
    const { success, fail, cancelled, unattempted } = finalProgress;
    const summary =
      ...
      (cancelled ? ` · 已停止 ${cancelled}` : "") +
      (unattempted ? ` · 未尝试 ${unattempted}` : "") +
      (targetTotal ? ` / 共 ${Math.max(targetTotal, success + fail + cancelled + unattempted)}` : "");
    ...
    setRegStatusText(
      cancelled > 0 && success === 0 && fail === 0
        ...
        : `成功 ${success} · 失败 ${fail}` +
            (cancelled ? ` · 停止 ${cancelled}` : "") +
            (unattempted ? ` · 未尝试 ${unattempted}` : "") +
            (targetTotal ? ` / ${Math.max(targetTotal, success + fail + cancelled + unattempted)}` : "")
    );
...
async function startDeviceLogin() {
  ...
  try {
    setDeviceProgress("starting", { phase: "running", label: "正在申请设备码…", pct: 12 });
...
function devicePhaseFromStatus(st) {
  const s = String(st || "").toLowerCase();
  if (!s || s === "idle" || s === "未开始") return "idle";
  if (["success", "completed", "imported", "done"].includes(s)) return "success";
  if (["error", "failed", "expired"].includes(s)) return "error";
  if (["authorization_pending", "pending", "waiting", "running", "starting", "queued"].includes(s)) return "running";
  if (/成功|完成|入库/.test(s)) return "success";
  if (/失败|过期|错误/.test(s)) return "error";
  if (/等待|进行|运行|验证|授权|pending/.test(s)) return "running";
  return "running";
}

function setDeviceProgress(statusText, opts = {}) {
  const phase = opts.phase || devicePhaseFromStatus(statusText);
  const wrap = $("device-progress-wrap");
  const fill = $("device-progress-fill");
  const bar = $("device-progress-bar");
  const label = $("device-progress-label");
  const pctEl = $("device-progress-pct");
  const live = $("device-live-dot");
  const statusEl = $("device-status");

  let pct = 0;
  let labelText = "点击「开始登录」获取设备码";
  if (phase === "idle") {
    pct = 0;
    labelText = "点击「开始登录」获取设备码";
  } else if (phase === "running") {
    // Device flow has no numeric progress — show a mid "waiting for auth" bar.
    pct = Math.max(18, Math.min(72, Number(opts.pct) || 42));
    labelText = opts.label || "等待浏览器完成验证…";
  } else if (phase === "success") {
    pct = 100;
    labelText = opts.label || "登录成功，账号已入库";
  } else if (phase === "error") {
    pct = 100;
    labelText = opts.label || "登录失败或已过期";
  }
  ...
  if (wrap) {
    wrap.dataset.devicePhase = phase;
    wrap.classList.toggle("is-live", phase === "running");
    wrap.classList.toggle("is-done", phase === "success");
    wrap.classList.toggle("is-error", phase === "error");
  }
  if (live) {
    live.classList.remove("is-live", "is-ok", "is-bad", "is-stop", "is-warn");
    if (phase === "running") live.classList.add("is-live");
    else if (phase === "success") live.classList.add("is-ok");
    else if (phase === "error") live.classList.add("is-bad");
  }
  if (statusEl) {
    const raw = statusText == null ? "未开始" : String(statusText);
    statusEl.textContent = raw;
    let state = "idle";
    if (phase === "success") state = "success";
    else if (phase === "error") state = "error";
    else if (phase === "running") state = "running";
    statusEl.setAttribute("data-device-state", state);
    statusEl.classList.add("g2a-reg-status-tag");
  }
}
...
function setDeviceLoginIdle(idle) {
  ...
  if (idle) {
    ...
    setDeviceProgress("未开始", { phase: "idle" });
  } else {
...
function showDeviceSession(r) {
  ...
  const statusText = msg ? (st + " · " + msg) : st;
  const phase = devicePhaseFromStatus(st);
  let label = msg || "";
  if (phase === "running") {
    label = code
      ? `设备码 ${code} · 请在浏览器完成验证`
    ...
    label = msg || "登录成功，账号已入库";
  } else if (phase === "error") {
    label = msg || "登录失败或已过期";
  }
  setDeviceProgress(statusText, {
    phase,
    label,
    pct: phase === "running" ? (code ? 48 : 28) : undefined,
  });
  ...
  if (!$("btn-start-reg").onclick) {
    on("btn-start-reg", "onclick", async () => {
      try {
        ...
        regBatchId = r.batch_id || null;
        {
          const startedCount = Number(r.count || (Array.isArray(r.session_ids) && r.session_ids.length) || 1) || 1;
          const workers = Number(r.concurrency || config.concurrency || 1) || 1;
          setRegStats({ total: startedCount, success: 0, fail: 0, running: Math.min(workers, startedCount), cancelled: 0 });
          if ($("btn-stop-reg")) $("btn-stop-reg").disabled = false;
          if ($("btn-stop-reg-inline")) $("btn-stop-reg-inline").disabled = false;
...
function paintUsageStatsGrid(sum, days) {
  ...
  const d = days || usageDays || 7;
  const failN = Number(today.fail || 0);
  const reqTone = failN > 0 && Number(today.success || 0) === 0 ? "bad" : (Number(today.requests || 0) > 0 ? "ok" : "warn");
  grid.innerHTML = `
    <div class="stat g2a-stat-card is-${reqTone}" data-stat="req">
      <div class="g2a-stat-top"><span class="label">今日请求</span><span class="g2a-stat-chip">req</span></div>
      <div class="value g2a-stat-hero">${fmtNum(today.requests)}</div>
      <div class="sub">成功 ${fmtNum(today.success)} · 失败 ${fmtNum(today.fail)}${today.success_rate != null ? ` · ${today.success_rate}%` : ""}</div>
    </div>
    <div class="stat g2a-stat-card is-ok" data-stat="tok-today">
      <div class="g2a-stat-top"><span class="label">今日计费 token</span><span class="g2a-stat-chip">tokens</span></div>
      <div class="value mono g2a-stat-hero">${fmtTokens(usageBilled(today))}</div>
      <div class="sub">输入 ${fmtNum(today.prompt_tokens_billed != null ? today.prompt_tokens_billed : Math.max(0, (Number(today.prompt_tokens)||0) - (Number(today.cache_read_tokens)||0)))} · 输出 ${fmtNum(today.completion_tokens)} · 已扣缓存 ${fmtNum(today.cache_read_tokens || 0)}</div>
    </div>
    <div class="stat g2a-stat-card" data-stat="cache-today">
      <div class="g2a-stat-top"><span class="label">今日缓存命中</span><span class="g2a-stat-chip">cache</span></div>
      <div class="value mono g2a-stat-hero">${fmtRatio(cacheToday.token_hit_ratio)}</div>
      <div class="sub">读 ${fmtNum(cacheToday.cache_read_tokens || 0)} / 输入 ${fmtNum(cacheToday.prompt_tokens || 0)} · 请求命中 ${fmtRatio(cacheToday.request_hit_ratio)}</div>
    </div>
    <div class="stat g2a-stat-card is-ok" data-stat="tok-window">
      <div class="g2a-stat-top"><span class="label">近 ${d} 天计费 token</span><span class="g2a-stat-chip">window</span></div>
      <div class="value mono g2a-stat-hero">${fmtTokens(usageBilled(window))}</div>
      <div class="sub">请求 ${fmtNum(window.requests)}${window.success_rate != null ? ` · 成功率 ${window.success_rate}%` : ""} · 已扣缓存 ${fmtNum(window.cache_read_tokens || 0)}</div>
    </div>
    <div class="stat g2a-stat-card" data-stat="cache-window">
      <div class="g2a-stat-top"><span class="label">近 ${d} 天缓存命中</span><span class="g2a-stat-chip">cache</span></div>
      <div class="value mono g2a-stat-hero">${fmtRatio(cacheWin.token_hit_ratio)}</div>
      <div class="sub">读 ${fmtNum(cacheWin.cache_read_tokens || 0)} / 输入 ${fmtNum(cacheWin.prompt_tokens || 0)} · 请求命中 ${fmtRatio(cacheWin.request_hit_ratio)}</div>
    </div>
    <div class="stat g2a-stat-card" data-stat="life">
      <div class="g2a-stat-top"><span class="label">累计计费 token</span><span class="g2a-stat-chip">life</span></div>
      <div class="value mono g2a-stat-hero">${fmtTokens(usageBilled(life))}</div>
      <div class="sub">请求 ${fmtNum(life.requests)} · 累计缓存读 ${fmtNum(cacheLife.cache_read_tokens || 0)} · 源 ${esc((sum && sum.source) || "—")}</div>
    </div>
  `;
}
...
function renderUsageBars(series) {
  ...
  host.innerHTML = rows.map((r, i) => {
    ...
    const h = Math.max(6, Math.round((tok / maxTok) * 100));
    const intensity = Math.max(0.35, Math.min(1, tok / maxTok));
    const isPeak = tok === maxTok && tok > 0;
    return `<div class="g2a-usage-bar${isPeak ? " is-peak" : ""}" style="--bar-h:${h}%;--bar-i:${intensity.toFixed(3)}" title="${esc(r.day)} · ${fmtTokens(tok)}（已扣缓存） · ${fmtNum(req)} 请求">
      <div class="g2a-usage-bar-val">${fmtTokens(tok)}</div>
      <div class="g2a-usage-bar-track">
        <div class="g2a-usage-bar-fill" style="height:${h}%"></div>
      </div>
      <div class="g2a-usage-bar-label">${esc(String(r.day || "").slice(5))}</div>
      <div class="g2a-usage-bar-req">${fmtNum(req)} req</div>
    </div>`;
  }).join("");
}
...
async function loadUsageSoft() {
  ...
  try {
    ...
    const cache = (sum && sum.cache) || {};
    paintUsageStatsGrid(sum, usageDays);
    if ($("usage-source")) {
