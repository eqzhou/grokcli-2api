...
**当前版本：v1.9.24（高并发 hybrid · 内联本地过盾）**
...
```bash
docker pull ghcr.io/hm2899/grokcli-2api:1.9.24
# 或
...
services:
  ...
  grokcli-2api:
    image: ghcr.io/hm2899/grokcli-2api:1.9.24
    ports:
      - "3000:3000"
    environment:
...
git tag v1.9.24
git push origin v1.9.24
```

成功后可拉取（**必须小写**）：

- `ghcr.io/hm2899/grokcli-2api:1.9.24`
...
- **v1.9.24**（当前）：修复注册/导入后账号池总数不刷新；账号列表服务端排序（默认最新在上）；开发态 `GROK2API_RELOAD` / `./dev.sh` 热更新
...
> 镜像 tag 与 `app.py` 中 `APP_VERSION` 一致（当前 **1.9.24**）。推 `main` 会打 `edge` 与版本号；打 `v1.9.24` tag 会额外发布 `latest`。  
